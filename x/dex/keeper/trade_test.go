...
func TestSingleTrade1(t *testing.T) {
	...
	require.NoError(t, cache.Transact(ctx, func(innerCtx sdk.Context) error {
		...
		stepCtx := tradeCtx.TradeToBase(fee, tradeCtx.OrdersCaches.ReserveFeeShare.Get())
	...
	require.NoError(t, cache.Transact(ctx, func(innerCtx sdk.Context) error {
		tradeCtx.Context = innerCtx
		stepCtx := tradeCtx.TradeToTarget(fee, tradeCtx.OrdersCaches.ReserveFeeShare.Get(), amount)
...
func TestSingleTrade3(t *testing.T) {
	...
	require.NoError(t, cache.Transact(ctx, func(innerCtx sdk.Context) error {
		...
		stepCtx = tradeCtx.TradeToTarget(fee, tradeCtx.OrdersCaches.ReserveFeeShare.Get(), amountReceived)
...
func TestSingleTrade8(t *testing.T) {
	...
	receivedAmount := k.ConstantProductTrade(ordersCache, utils.BaseCurrency, "ukusd", *maximum).RoundInt()
...
func TestTrade15(t *testing.T) {
	...
	require.Equal(t, 8.00768737988469, price)
...
func TestTrade31(t *testing.T) {
	...
	require.NoError(t, cache.Transact(ctx, func(innerCtx sdk.Context) error {
		...
		stepCtx = tradeCtx.TradeToTarget(fee, tradeCtx.OrdersCaches.ReserveFeeShare.Get(), amountReceivedNet)
		_, _, _, err = k.ExecuteTradeStep(stepCtx)
		return err
	}))
